...
function saveProduct() {
    ...
    const product = {
        ...
        stock: editId ? appState.products.find(p => p.id === editId)?.stock || 0 : 0
    };

    if (editId) {
        const index = appState.products.findIndex(p => p.id === editId);
        if (index !== -1) {
...
function filterProducts() {
    ...
    const filtered = appState.products.filter(p => {
        ...
        const matchFilter = !filterValue || p.category === filterValue;
...
function renderProductsTable(products = appState.products) {
...
function renderLocationsTable() {
    ...
    tbody.innerHTML = appState.locations.map(location => `
        <tr>
            <td>${location.name}</td>
            <td>${location.code}</td>
...
function renderSuppliersTable() {
    ...
    tbody.innerHTML = appState.suppliers.map(supplier => `
        <tr>
            <td>${supplier.name}</td>
...
function renderReceiptsTable() {
    ...
    tbody.innerHTML = appState.receipts.map(receipt => `
        <tr>
            <td>${receipt.code}</td>
...
function renderIssuesTable() {
    ...
    tbody.innerHTML = appState.issues.map(issue => `
        <tr>
            <td>${issue.code}</td>
...
function updateDashboard() {
    ...
    document.getElementById('totalLocations').textContent = appState.locations.length;
    
    const today = new Date().toISOString().split('T')[0];
    const todayReceipts = appState.receipts.filter(r => r.date === today).length;
    const todayIssues = appState.issues.filter(i => i.date === today).length;
    
    document.getElementById('todayReceipts').textContent = todayReceipts;
    document.getElementById('todayIssues').textContent = todayIssues;
}
...
function loadSampleData() {
    ...
    if (appState.products.length === 0) {
        appState.products = [
            { id: '1', sku: 'SP001', name: 'Sản phẩm A', unit: 'Cái', price: 100000, stock: 50, category: 'Danh mục 1' },
            { id: '2', sku: 'SP002', name: 'Sản phẩm B', unit: 'Thùng', price: 500000, stock: 20, category: 'Danh mục 2' }
        ];
    }
    ...
    if (appState.locations.length === 0) {
        appState.locations = [
            { id: '1', name: 'Kho A', code: 'KHO-A', capacity: 1000, used: 500 },
            { id: '2', name: 'Kho B', code: 'KHO-B', capacity: 800, used: 300 }
        ];
    }
    ...
    if (appState.suppliers.length === 0) {
        appState.suppliers = [
            { id: '1', name: 'Nhà cung cấp ABC', taxCode: '123456789', address: '123 Đường XYZ', phone: '0123456789', email: 'abc@example.com' }
        ];
    }
    ...
    if (appState.customers.length === 0) {
        appState.customers = [
            { id: '1', name: 'Khách hàng XYZ', taxCode: '987654321', address: '456 Đường ABC', phone: '0987654321', email: 'xyz@example.com' }
        ];
    }
    ...
    if (appState.users.length === 0) {
        appState.users = [
            { id: '1', username: 'admin', fullName: 'Quản trị viên', email: 'admin@example.com', role: 'admin', status: 'active' }
        ];
    }
    ...
    if (appState.vehicles.length === 0) {
        appState.vehicles = [
            { id: '1', plate: '29A-12345', type: 'truck', driver: 'Nguyễn Văn A', driverPhone: '0123456789', owner: 'Công ty ABC', status: 'active', createdAt: new Date().toISOString() },
            { id: '2', plate: '30B-67890', type: 'container', driver: 'Trần Thị B', driverPhone: '0987654321', owner: 'Công ty XYZ', status: 'active', createdAt: new Date().toISOString() }
        ];
    }
}
